...
门户手工部署一次(`docker/deploy-ide-portal.sh`,配置 `docker/ide-portal/portal.example.yaml`),挂在同一个代理后面,`VIRTUAL_HOST=ide.jereh-pe.cn`;它不自我开通。它绑 `bindHost: 0.0.0.0`,因为代理是经 `dc_default` 访问容器的(0005)——只绑回环是 dsh web 的规则,不是所有服务的。控制台通道意味着 Jenkins 失联时直播冻结、事实不冻结:下一次 reconcile 重读 Docker 状态。标记文件恢复是尽力而为:门户停机期间完成的构建,会在下一次进入时以普通 reconcile 呈现。启动页是零框架构建的静态 SPA(`web/`),不用打包器就满足了前后端分离。入口在到达时自动做检查:reconcile 探测是只读的(只读 Docker 状态,不改任何东西),所以无需点击即运行(需求方拍板,2026-09-06:只读操作全部自动化;这取代并移除了此前的 `autoCheck` 双模式开关)。探测结果直接渲染在页面上——容器健康时状态栏显示就绪并出现打开按钮,容器缺席时出现检查按钮;页面和入口都不会自行开通或跳转:开通是检查按钮的点击,进入 IDE 是打开按钮的点击。就绪状态停在状态栏上——页面绝不自行跳转,进入 IDE 是用户点击"打开我的 IDE"按钮那一刻才发生的(需求方拍板,2026-09-06:先展示状态,点击才跳)。服务器够不到 IAM 时,`iam.trustFile` 指向一个 JSON 文件,内容是提供方公开的两份文档(`discovery`、`jwks`),从任何够得到 IAM 的网络抓下来即可:门户此后不再 fetch,登录与验签在灌入的 issuer、端点与密钥上完成(运维在 IAM 轮换密钥后重抓该文件,否则新密钥签出的 token 会被拒绝)。随附的闸门接受同一个文件作为 `trustFile`(读 `IAM_TRUST_FILE`,默认取挂载的 `/etc/ide-portal/iam-trust.json`),`provision.sh` 把宿主机灌好的文件只读挂进每个用户容器——够不到 IAM 的容器网络也能完全离线完成登录与验签。测试用本地 HTTP 服务器上的真实 RSA JWKS 冒充 IAM,用永不发起 fetch 的离线路径钉死离线验签,用脚本化标记构建冒充 Jenkins;`tests/e2e.spec.ts` 经回环驱动真实 CLI 进程,对面是真实套接字上的两个假件——登录往返、检查按钮到 READY 的完整流程含 SSE 回放、stopped 与 healthy 两条短路、常开的只读入口检查、标记文件重挂、离线信任文件启动及其 issuer 不一致的 fail-loud 拒绝——门户内部零测试缝,任何凭证不落盘;`docker/ide-provision/provision.sh` 用 `docker`/`curl` 假件做 hermetic 演练;真实宿主的冷启动是 0008 发布步骤 2。用户容器随镜像带上了随附的 IAM 闸门(O2):`docker/dsh-aio/dshhome/iam-gate.cordis.patch.yml` 在 `DSH_IAM_GATE=1`(开通脚本固定注入)时经 entrypoint 的 `--patch` 层挂上 `auth-iam`,健康探测接受闸门的 `302`/`401` 应答;没带这个开关的容器保持开放,既有部署不受影响。闲置回收(O4)、TLS(O5)、资源上限(O7)按需求方指示继续搁置。
