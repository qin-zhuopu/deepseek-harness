...
该包不了解任何 harness 概念，也不提供任何文件服务：`/api` HTTP 桥接与下行 WebSocket 是 connection 插件的 route，插件 bundle 与 HMR（热模块替换）事件流是 modules／hmr 插件的 route，dist 服务则属于 fallback 持有者。upgrade handler 拥有协议握手与连接内容；webserver 只交付原始 socket 与 request。`registerGuard(guard)` 与 `registerUpgradeGuard(guard)` 添加闸门（各自返回 disposer；闸门可叠加，按注册顺序接受咨询）：`WebGuard` 在每条具名 route 与 fallback handler 之前运行，并被告知所走的 surface（`'route'` 或 `'fallback'`）——第一个拒绝终结链条并拥有自己的响应（未写响应的拒绝得到空的 401）；`UpgradeGuard` 在 upgrade route 表之前运行，拒绝裁决（`{status, headers?}`）会在 socket 被销毁前直接答复挂起的 upgrade，因此拒绝不会泄露该 pathname 是否有所有者。闸门就是认证 seam：随附的守门者是 [`dsh-host-auth-jwt`](../auth-jwt/README.zh.md)（HS256 共享密钥 token）与 [`dsh-host-auth-iam`](../auth-iam/README.zh.md)（对照提供方 JWKS 验签的企业 OIDC id_token），两者都经由共享的 [`dsh-host-auth-core`](../auth-core/README.zh.md) 挂载。未命中任何具名 route 且 fallback 席位空置的请求是裸 404，不经过任何闸门。`host` 只接受 `127.0.0.1`（默认安全姿态）和 `0.0.0.0`（有意向网络开放）。该服务器只服务浏览器；Electron 通过 `file://` 加载 dist，并经 IPC 桥接承载 fetch。该包从不打印内容；URL 行属于 shell。
...
- **不提供 TLS 或来源策略**：绑定非回环地址会向对应网络公开服务器；认证已作为可自选的闸门持有者存在（[`dsh-host-auth-jwt`](../auth-jwt/README.zh.md)、[`dsh-host-auth-iam`](../auth-iam/README.zh.md)），但传输加密（或在前方放置真正的反向代理）仍属有意不纳入面向开发环境 v1 的部署加固。
