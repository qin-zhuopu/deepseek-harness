...
| [`auth-core/`](auth-core/README.zh.md) | 认证闸门共享的闸门/cookie/登录机制 | 供闸门持有者使用的库 |
| [`auth-jwt/`](auth-jwt/README.zh.md) | 覆盖 webserver 全部请求面的可选 JWT bearer 闸门 | 消费 `ctx.webServer` |
| [`auth-iam/`](auth-iam/README.zh.md) | 覆盖 webserver 全部请求面的可选企业 OIDC 闸门（JWKS 验签 id_token） | 消费 `ctx.webServer` |
