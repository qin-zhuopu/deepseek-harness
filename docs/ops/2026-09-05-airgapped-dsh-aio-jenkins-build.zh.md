...
## 经 nginx-proxy 暴露域名（dsh.jereh-pe.cn）

通配 `*.jereh-pe.cn` 解析到 17.58；`jr-nginx-proxy` 跑在 `/home/admin/git/dc/docker-compose.yml` 的 compose 默认网络 `dc_default` 上（docker-compose v1 1.29.2）。compose 里原有的 `dsh-aio` 服务（prod 镜像、容器名 `dsh-aio-dc`、vhost `dsh.jr.zhuopu.net`、未在运行）就地改写为新镜像与新域名；原文件已备份为同目录 `docker-compose.yml.bak.<时间戳>`，`NR_API_KEY` 环境变量行原样保留。

- jc 已注册实例：`jc nginx-proxy add --name zhuopu1758 --host 10.1.17.58 --user admin --compose-dir /home/admin/git/dc --domain-suffix "*.jereh-pe.cn"`。`jc nginx-proxy service add` 只会生成最小服务块（image/VIRTUAL_HOST/VIRTUAL_PORT），所以服务块按 0005 的完整配方手写；jc 的 `service list/up` 对同一文件照常可用。本容器没有 `ssh` 可执行文件，所有远端改动都借 Jenkins 的 `ssh` 凭据以 base64 上传脚本的方式执行。
- compose 内的 PID1 冻结处理：`entrypoint: ["bash","-c","sleep 60000"]` 加 bind 挂载 `/home/admin/dsh-aio-supervise.sh`（存在 `/tmp/entrypoint.log` 就退出，否则后台拉起 `entrypoint.sh`）。`docker restart dsh-aio` 单独执行即可在约 45 秒后恢复 web；`docker-compose up -d` 新建容器后需要 exec 一次这个 hook。
- 环境变量（0005 配方）：`FRONT_PORT=8080`、`VNC_PUBLIC_URL=/vnc`、`RESIZE_ENDPOINT=/resize`、`TRUSTED_HOSTS=dsh.jereh-pe.cn`、`VIRTUAL_HOST=dsh.jereh-pe.cn`、`VIRTUAL_PORT=8080`、`HTTPS_METHOD=noredirect`。
