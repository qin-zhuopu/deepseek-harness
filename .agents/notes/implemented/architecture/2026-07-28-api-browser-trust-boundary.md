...
Two boundaries stay deliberately out of scope: reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), and authentication for genuinely remote deployments was deferred work at this decision — the fence is a confused-deputy defense, not an auth layer; a mounted gate now participates only at the pin's loopback check and the prefix fence stays the plain authority comparison ([the auth gate admits the privileged plane](2026-09-05-auth-gate-admits-the-privileged-plane.md)). The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
