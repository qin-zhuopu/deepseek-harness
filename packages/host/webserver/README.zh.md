...
- **不提供 TLS 或来源策略**：绑定非回环地址会向对应网络公开服务器；认证已作为可自选的闸门持有者存在（[`dsh-host-auth-jwt`](../auth-jwt/README.zh.md)），但传输加密（或在前方放置真正的反向代理）仍属有意不纳入面向开发环境 v1 的部署加固。
