...
| `POST /api/check` | 重复到达检查,只读;"检查我的IDE"按钮驱动,检查链走 SSE。 |
| `POST /api/provision` | 开通,幂等(需求方拍板,2026-09-06):HEALTHY 短路、进行中的开通加入而非重起(FR7)、只有 absent/stopped 才真正 create/start;"启动我的IDE"按钮驱动,覆盖失败重试。 |
...
入口探测是唯一自动发生的动作(只读;需求方拍板,2026-09-06:只读操作无需点击)。三个按钮(需求方拍板,2026-09-06):"检查我的IDE"重复只读探测,"启动我的IDE"幂等启动(create/start/重试收敛为一次动作),"进入我的IDE"跳转;每次检查渲染为一条链——工号、域名、服务状态、Compose 位置、健康检查、结论,detail 为中文可读文案,新链开始即替换旧链。就绪状态停在状态栏上——浏览器绝不自行跳转;常驻的"进入我的IDE"按钮承载跳转。
